...
namespace Application.Models
{
    public class IssueDto(int Id, int? BookingId, int? TipoDeReclamo, string? TipoDeReclamoNombre, string Texto, string? Imagen, string Estado, DateTime CreatedAtUtc)
    {
        ...
        {
            return new IssueDto(issue.Id, issue.BookingId, issue.MaintenanceTypeId, issue.MaintenanceType?.Description, issue.Text, issue.Image, issue.Status, issue.CreatedAtUtc);
        }
    }
}
